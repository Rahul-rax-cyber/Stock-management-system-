...
export default function App() {
  ...
  const handleSaveMasterItems = (updatedItems: MasterItem[]) => {
    ...
    // Also update current log's stock items if there are any new items
    if (currentLog) {
      const updatedStockItems = updatedItems.map((item) => {
        const existing = currentLog.stockItems.find((s) => s.itemId === item.id);
        if (existing) {
          return {
            ...existing,
            itemName: item.name,
            itemPrice: existing.itemPrice || item.defaultPrice,
          };
        }
        return {
          itemId: item.id,
      ...
      <aside className="w-64 bg-slate-900 flex flex-col text-white shrink-0 shadow-2xl hidden md:flex">
        ...
        <div className="p-6 border-t border-slate-800">
          <div className="bg-slate-800 p-4 rounded-xl">
            <p className="text-xs text-slate-400 uppercase font-bold tracking-widest mb-2">Project Status</p>
            <div className="flex items-center space-x-2">
              <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></div>
              <p className="text-sm font-medium text-slate-200">CloudApp v2.4.0</p>
            </div>
          </div>
        </div>
      </aside>
